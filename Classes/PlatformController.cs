...
namespace Doodle_Jump.Classes
{
    public static class PlatformController
    {
        ...
        public static void AddPlatforms(PointF position)
        {
        ...
        public static void GenerateStartSequence() // доб опред кол-во платформ на карту
        {
            Random r = new Random();
            for (int i = 0;i<10; i++)
            {
                int x = r.Next(0, 270);
                int y = r.Next(50, 60);
                startPlatformPosY -= y;// смещение т.е. с каждой генерацией платформа выше
        ...
        public static void GenerateRandomPlatform() // добавление платформ в рандомных местах
        {
            ClearPlatforms();
            Random r = new Random();
            int x = r.Next(0, 270);
            ...
            var c = r.Next(1, 3);
            switch (c) // чтобы не спвнлись одновременно
            {
                case 1:
                    //рандомное создание врага
                    c = r.Next(1, 10);
                    if (c == 1)
                    {
        ...
        public static void CreateBonus(Platform platform)
        {
            ...
            {
                case 1:
                    var bonus = new Bonus(new PointF (platform.transform.position.X + (platform.sizeX / 2) - 15, platform.transform.position.Y - 30), bonusType); // -15 чтоб посередине платформы и -30 чтобы над ней
                    bonuses.Add(bonus);

                    break;
                case 2:
                    bonus = new Bonus(new PointF(platform.transform.position.X + (platform.sizeX / 2) - 7, platform.transform.position.Y - 15), bonusType);
                    bonuses.Add(bonus);
                    break;
            }
        }
        ...
        public static void ClearPlatforms() // очистка платформ, если они далеко 
        {
